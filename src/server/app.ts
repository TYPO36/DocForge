...
export function createApp(storage: Storage, authEnv?: AuthEnv, scheduler: TaskScheduler = detachedScheduler, vectorIndex: VectorIndex | null = null) {
  ...
  app.route("/api/documents", documentsRoutes(storage, scheduler, vectorIndex));
  app.route("/api/chat", chatRoutes(storage, vectorIndex));
